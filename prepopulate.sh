...
# Define base directory (matches network volume mount)
BASE_DIR="/workspace"
...
SENTINEL_FILE="${BASE_DIR}/comfyui/.setup_complete"

# Create directories (only for dynamic components, static dirs handled by image)
...
clone_repo() {
    ...
    if [ -f "$target_dir/requirements.txt" ]; then
        echo "Collecting requirements for $folder_name..."
        cat "$target_dir/requirements.txt" >> "${BASE_DIR}/comfyui/all_requirements.txt"
    fi
}

# Clone ComfyUI itself
if [ ! -d "${BASE_DIR}/comfyui" ]; then
    echo "Cloning ComfyUI into ${BASE_DIR}/comfyui..."
    git clone https://github.com/comfyanonymous/ComfyUI.git "${BASE_DIR}/comfyui"
    cd "${BASE_DIR}/comfyui" || exit 1
    git checkout 96d891cb94d90f220e066cebad349887137f07a6
    cd - >/dev/null
    cat "${BASE_DIR}/comfyui/requirements.txt" >> "${BASE_DIR}/comfyui/all_requirements.txt"
else
    echo "ComfyUI already exists, skipping clone..."
fi

# List of custom nodes with specific commits
...
clone_repo "https://github.com/ShmuelRonen/ComfyUI-Apply_Style_Model_Adjust.git" "21d3b35bfb5d7922b0b2656a1d4626ecd7de5121"
...
clone_repo "https://github.com/ltdrdata/ComfyUI-Impact-Pack.git" "66493d8cb4209e0ad19c914ae1e39ee7e1486d7e"
...
echo "Prepopulation complete! Static files (handler.py, workflow.json, etc.) are assumed to be in the Docker image."
touch "$SENTINEL_FILE"
echo "Created sentinel file: $SENTINEL_FILE"